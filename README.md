...
QwenVL-RFT/
  ...
  scripts/
    data_process/             # 数据分析 notebook
  src/qwen_vl_rl/
    algorithms/               # 答案、reward、PPO、GRPO 数学逻辑
    cli/                      # 完整命令行与训练编排
    config/                   # 配置 schema、加载和校验
    data/                     # Dataset、划分和 collator
    models/                   # Qwen-VL、LoRA、value head
    reporting/                # 预测报告和训练曲线
    training/                 # 训练循环、指标、checkpoint I/O
    utils.py
  tests/
  pyproject.toml
  README.md
...
| 命令 | 功能 |
| --- | --- |
| `qwen-vl-sft` | SFT warm start |
| `qwen-vl-ppo` | PPO LoRA 训练与测试 |
| `qwen-vl-grpo` | GRPO LoRA 训练与测试 |
| `qwen-vl-convert-thyme` | Thyme-SFT 数据转换 |
| `qwen-vl-plot` | 重画训练指标曲线 |

`scripts/data_process/` 仅保留原始数据分析和转换结果检查 notebook。

### 核心模块

| 目录 | 功能 |
| --- | --- |
| `src/qwen_vl_rl/algorithms/` | 答案格式、reward、PPO/GRPO rollout 与 loss |
| `src/qwen_vl_rl/data/` | JSONL 数据集、固定划分、SFT/PPO/GRPO collator |
| `src/qwen_vl_rl/models/` | dtype、量化、LoRA、reference model 和 PPO value head |
| `src/qwen_vl_rl/training/` | 公共 RL 循环、严格评估指标、续训和 checkpoint 管理 |
| `src/qwen_vl_rl/reporting/` | JSONL/HTML 测试报告和训练曲线 |
| `src/qwen_vl_rl/cli/` | SFT/PPO/GRPO、转换和绘图命令的完整实现 |
...
# 仅下载数据、运行 notebook 时需要
pip install datasets==4.8.5 modelscope plotly sentencepiece trl==1.3.0
```
...
如果机器上有多张 GPU，直接运行 `qwen-vl-sft`、`qwen-vl-ppo` 或 `qwen-vl-grpo` 通常只会使用单进程的 `cuda:0`。需要多卡训练时，应使用 `accelerate launch --num_processes <N> -m qwen_vl_rl.cli.<module>` 启动。
...
```bash
accelerate launch --num_processes 4 -m qwen_vl_rl.cli.train_sft \
  --config configs/sft_qwen_vl_lora.yaml
```
...
```bash
accelerate launch --num_processes 4 -m qwen_vl_rl.cli.train_ppo \
  --config configs/ppo_qwen_vl_lora.yaml
```
...
```bash
accelerate launch --num_processes 4 -m qwen_vl_rl.cli.train_grpo \
  --config configs/grpo_qwen_vl_lora.yaml
```
...
```bash
qwen-vl-plot outputs/sft/default --kind sft
qwen-vl-plot outputs/ppo/default --kind ppo --rolling-window 20
qwen-vl-plot outputs/grpo/default --kind grpo --rolling-window 20
```
...
```bash
pytest tests/test_convert_thyme.py
